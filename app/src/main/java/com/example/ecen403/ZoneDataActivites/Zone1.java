...
public class Zone1 extends AppCompatActivity {
    ...
    String rtvTemp, rtvMoisture, rtvPrecipitation, rtvStatus, rtvHumidity, zone1, water_amount, time_active, rate_speed;
    ...
    private DatabaseHelper databaseHelper; // Declare the database helper at class level
    private WaterAmountDatabaseHelper waterAmountDatabaseHelper;
    private RateSpeedDatabaseHelper rateSpeedDatabaseHelper;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        waterAmountDatabaseHelper = new WaterAmountDatabaseHelper(this);
        rateSpeedDatabaseHelper = new RateSpeedDatabaseHelper(this);

        txtTemp = findViewById(R.id.rtvTemp);
        txtMoisture = findViewById(R.id.rtvMoisture);
        txtHumidity = findViewById(R.id.rtvHumidity);
        txtPrecipitation = findViewById(R.id.rtvPrecipitation);
        txtStatus = findViewById(R.id.rtvStatus);
        ...
        swipeRefreshLayout = findViewById(R.id.swipeRefreshLayout); // Ensure this ID matches your layout
        ...
        db.collection("zones")
                ...
                .addOnFailureListener(new OnFailureListener() {
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(Zone1.this, "Error = " + e.getMessage(), Toast.LENGTH_SHORT).show();
                        swipeRefreshLayout.setRefreshing(false); // Stop the refreshing indicator
                    }
                });
                            ...
                            if (documentSnapshot.exists()) {
                                ...
                                if (timeActiveStr != null && previousValueStr != null && waterAmountStr != null && previousWaterAmountStr != null && rateSpeedStr != null && previousRateSpeedStr != null) {
                                    try {
                                        ...
                                        databaseHelper.insertData(dateInMillis, timeActive); // Save to database
                                        waterAmountDatabaseHelper.insertData(dateInMillis, waterAmount);
                                        rateSpeedDatabaseHelper.insertData(dateInMillis, rateSpeed);

                                        // Check if the values are different
                                        if (timeActive != previousValue) {
                                            // Values are different, so update the date and previous_value
                                            updateDateAndPreviousValue("stored1", timeActive);
                                        } else {
                                        ...
                                        if (waterAmount != previousWaterAmount) {
                                            // Values are different, so update the date and previous_value
                                            updateDateAndPreviousWaterValue("stored1", waterAmount);
                                        } else {
                                            // Values are the same, no update needed
                                            Log.d("Firestore Check", "No update needed, values are the same.");
                                        }
        ...
        db.collection("zones")
                ...
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        Toast.makeText(Zone1.this, "Status updated!", Toast.LENGTH_SHORT).show();
                    }
                })
                .addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(Zone1.this, "Error updating status: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    private void appendDataToWaterGraph(double dateInMillis, double waterAmount) {
        ...
        waterAmountGraph.invalidate();
        waterAmountDatabaseHelper.insertData(dateInMillis, waterAmount); // Save to database
    }
    ...
    private void appendDataToRateSpeedGraph(double dateInMillis, double rateSpeed) {
        ...
        rateSpeedGraph.invalidate();
        rateSpeedDatabaseHelper.insertData(dateInMillis, rateSpeed); // Save to database
    }
    ...
    private void loadDataFromWaterAmountDatabase() {
        ...
        Cursor cursor = waterAmountDatabaseHelper.getAllData();
    ...
    private void loadDataFromRateSpeedDatabase() {
        Cursor cursor = rateSpeedDatabaseHelper.getAllData();
